...
function renderInvoice(
  ...
  // ── Header ──────────────────────────────────────────────────
  doc.font("Helvetica-Bold").fontSize(20).fillColor(brandColor).text("ZAYELLE", left, y);
  doc.font("Helvetica").fontSize(8.5).fillColor(mutedColor).text("Premium Hijabs & Modest Accessories", left, y + 23);
  ...
  doc.font("Helvetica").fontSize(8).fillColor(mutedColor)
    .text(copyLabel, right - 150, y + 20, { width: 150, align: "right" });
  ...
  doc.font("Helvetica-Bold").fontSize(7.5).fillColor(mutedColor).text("ORDER ID", left, y);
  doc.font("Helvetica").fontSize(9.5).fillColor(textColor).text(order.orderId, left, y + 10);

  doc.font("Helvetica-Bold").fontSize(7.5).fillColor(mutedColor).text("DATE", left + colW, y);
  doc.font("Helvetica").fontSize(9.5).fillColor(textColor).text(invoiceDate, left + colW, y + 10);
  ...
  doc.font("Helvetica-Bold").fontSize(7.5).fillColor(mutedColor).text("PAYMENT", left + 2 * colW, y);
  doc.font("Helvetica-Bold").fontSize(9.5).fillColor(statusColor).text(payStatusText, left + 2 * colW, y + 10);
  ...
  doc.font("Helvetica-Bold").fontSize(7.5).fillColor(brandColor).text("BILL TO", left, y);
  doc.font("Helvetica-Bold").fontSize(10).fillColor(textColor).text(order.customerName, left, y + 11, { width: halfW });
  doc.font("Helvetica").fontSize(8.5).fillColor(mutedColor).text(order.customerEmail, left, y + 23, { width: halfW });
  if (order.customerPhone) {
    doc.font("Helvetica").fontSize(8.5).fillColor(mutedColor).text(order.customerPhone, left, y + 33, { width: halfW });
  }

  doc.font("Helvetica-Bold").fontSize(7.5).fillColor(brandColor).text("SHIP TO", left + halfW + 12, y);
  if (order.shippingAddress) {
    doc.font("Helvetica").fontSize(8.5).fillColor(textColor)
      .text(order.shippingAddress, left + halfW + 12, y + 11, { width: halfW });
  }
  ...
  y += 11;
  doc.font("Helvetica-Bold").fontSize(9).fillColor(textColor).text("Zayelle", left, y);
  y += 12;
  doc.font("Helvetica").fontSize(8.5).fillColor(textColor)
    .text("Thoppumpady Post Office, Kochi, Kerala \u2013 682005", left, y);
  y += 11;
  doc.font("Helvetica").fontSize(8.5).fillColor(textColor).text("Contact: 8891485648  |  zayelle.in@gmail.com", left, y);
  y += 14;
  ...
  doc.text("PRICE", c3, y + 4, { width: 58, align: "right" });
  ...
  for (let idx = 0; idx < order.items.length; idx++) {
    ...
    if (cfgLines.length > 0) {
      ...
      doc.font("Helvetica").fontSize(7).fillColor(mutedColor)
        .text(cfgText, c1, y + 3 + nameH, { width: nameW, lineGap: 0 });
      cfgH = doc.heightOfString(cfgText, { width: nameW, lineGap: 0 });
    }

    doc.font("Helvetica").fontSize(9).fillColor(textColor)
      .text(String(item.quantity), c2, y + 3, { width: 50, align: "center" });
    doc.text(rupees(parseFloat(item.price)), c3, y + 3, { width: 58, align: "right" });
  ...
  doc.font("Helvetica").fontSize(8.5).fillColor(mutedColor).text("Subtotal:", sumX, y, { width: 58, align: "right" });
  doc.font("Helvetica").fontSize(8.5).fillColor(textColor).text(rupees(subtotal), sumValX, y, { width: 55, align: "right" });
  y += 12;

  if (discount > 0) {
    doc.font("Helvetica").fontSize(8.5).fillColor(mutedColor).text("Discount:", sumX, y, { width: 58, align: "right" });
    doc.font("Helvetica").fontSize(8.5).fillColor("#2E7D32").text(`-${rupees(discount)}`, sumValX, y, { width: 55, align: "right" });
    y += 12;
  }

  doc.font("Helvetica").fontSize(8.5).fillColor(mutedColor).text("Shipping:", sumX, y, { width: 58, align: "right" });
  doc.font("Helvetica").fontSize(8.5).fillColor(textColor).text("FREE", sumValX, y, { width: 55, align: "right" });
  y += 12;

  if (codCharge > 0) {
    doc.font("Helvetica").fontSize(8.5).fillColor(mutedColor).text("COD Charges:", sumX, y, { width: 58, align: "right" });
    doc.font("Helvetica").fontSize(8.5).fillColor(textColor).text(rupees(codCharge), sumValX, y, { width: 55, align: "right" });
    y += 12;
  }
