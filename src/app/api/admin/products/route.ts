...
export async function GET() {
  ...
  const formatted = allProducts.map((p) => ({
    ...
    compareAt: p.compareAt ? parseFloat(p.compareAt) : undefined,
    image: p.image,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const [newProduct] = await db.insert(products).values({
      ...
      compareAt: body.compareAt ? body.compareAt.toString() : null,
      image: body.image,
    ...
    return NextResponse.json({
      ...
      compareAt: newProduct.compareAt ? parseFloat(newProduct.compareAt) : undefined,
